...
export function SidangScreen() {
  const [data, setData] = useState<Sidang | null>(null);
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const response = await sidangApi.myStatus();
      if (response.data.success) {
        setRegistered(response.data.data.registered);
        setData(response.data.data.sidang ?? null);
      }
  ...
  const handleRegister = async () => {
    try {
      const lembarRevisi = await pickFile('lembar_revisi');
      if (!lembarRevisi) return;
      const draftTa = await pickFile('draft_ta');
      if (!draftTa) return;
      ...
      formData.append('lembar_revisi', lembarRevisi as unknown as Blob);
      formData.append('draft_ta', draftTa as unknown as Blob);
      formData.append('bukti_plagiasi', buktiPlagiasi as unknown as Blob);

      await sidangApi.register(formData);
      Alert.alert('Berhasil', 'Pendaftaran sidang berhasil');
      fetchStatus();
    } catch (err: unknown) {
      const error = err as { message?: string; errors?: Record<string, string[]> };
      const msg = error.errors
        ? Object.values(error.errors).flat().join('\n')
        : error.message ?? 'Gagal mendaftar sidang';
    ...
    >
      {!registered ? (
        <View style={styles.registerCard}>
          <Text style={styles.registerTitle}>Daftar Sidang Tugas Akhir</Text>
          <Text style={styles.registerDesc}>
            Upload dokumen berikut untuk mendaftar sidang:{'\n'}
            1. Lembar Revisi (PDF, maks 5MB){'\n'}
            2. Draft TA (PDF, maks 20MB){'\n'}
            3. Bukti Plagiasi (PDF, maks 5MB)
          </Text>
          <TouchableOpacity
